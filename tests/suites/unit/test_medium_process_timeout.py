"""Contract tests for medium process-timeout orchestration helper."""
...
from ser._internal.runtime import medium_process_timeout
...
def test_run_with_process_timeout_completes_setup_and_inference_phases() -> None:
    """Helper should mark setup/inference completion and return parsed result."""
    ...
    result = medium_process_timeout.run_with_process_timeout(
        payload=_Payload(expected_backend_model_id="facebook/wav2vec2-xls-r-300m"),
        profile="medium",
        timeout_seconds=11.0,
        get_context=lambda _name: object(),
        logger=logging.getLogger("ser.tests.medium_process_timeout"),
    ...
    assert phase_started == ["setup:medium", "inference:medium"]
    assert phase_completed == ["setup:medium", "inference:medium"]
    assert handshake_kwargs["timeout_seconds"] == 11.0


def test_run_with_process_timeout_logs_inference_failure_on_parse_error() -> None:
    """Helper should emit inference phase failure when message parse fails."""
    ...
    with pytest.raises(RuntimeError, match="parse failed"):
        medium_process_timeout.run_with_process_timeout(
            payload=_Payload(expected_backend_model_id="facebook/wav2vec2-xls-r-300m"),
            profile="medium",
            timeout_seconds=9.0,
            get_context=lambda _name: object(),
    ...
    assert phase_failed == ["inference:medium"]
