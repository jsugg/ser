"""Tests for medium worker lifecycle adapters."""
...
def test_run_with_process_timeout_delegates_to_timeout_helper(
    ...
    monkeypatch.setattr(
        medium_worker_lifecycle.medium_process_timeout_helpers,
        "run_with_process_timeout",
        lambda **kwargs: captured.update(kwargs) or expected,
    )

    result = medium_worker_lifecycle.run_with_process_timeout(
        payload="payload",
        profile="medium",
        timeout_seconds=7.0,
        get_context=lambda _name: object(),
        logger=logging.getLogger("ser.tests.medium_worker_lifecycle"),
...
def test_run_worker_entry_delegates_to_binding(
    ...
    monkeypatch.setattr(
        medium_worker_lifecycle,
        "_run_worker_entry_binding",
        lambda **kwargs: captured.update(kwargs),
    )

    medium_worker_lifecycle.run_worker_entry(
        payload,
        connection,
...
def test_raise_worker_error_delegates_to_binding(
    ...
    """Lifecycle owner should wire medium-specific worker error mapping."""
    captured: dict[str, object] = {}
    ...
    medium_worker_lifecycle.raise_worker_error(
        "RuntimeError",
        "boom",
...
def test_terminate_worker_process_delegates_to_binding(
    ...
    medium_worker_lifecycle.terminate_worker_process(
        process,
        impl=lambda **_kwargs: None,
