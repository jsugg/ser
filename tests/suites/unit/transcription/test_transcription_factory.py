...
def test_factory_resolves_faster_adapter_without_importing_stable_backend() -> None:
    ...
    module_names = (
        "ser._internal.transcript.backends.factory",
    ...
    try:
        factory = importlib.import_module("ser._internal.transcript.backends.factory")
        reloaded_factory = importlib.reload(factory)

        assert "ser._internal.transcript.backends.stable_whisper" not in sys.modules
        assert "ser._internal.transcript.backends.stable_whisper_mps_compat" not in sys.modules
